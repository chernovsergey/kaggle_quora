...
INPUT_FNAME = '../../data/train.csv'
OUTPUT_NAME = '../../data/nltk_similarity_score_train.csv'
import pandas as pd
...
train_no_nan = data.fillna(value='')
train_data_clean = clean_dataframe(train_no_nan)

assert len(data) == len(train_data_clean)
print 'Clean up - ok'

sentences1 = train_data_clean.question1.values
sentences2 = train_data_clean.question2.values
assert len(sentences1) == len(sentences2)
sent_pairs = zip(sentences1, sentences2)
...
def worker(pair):
    try:
        score = semantic_similarity(pair[0].decode('utf-8'), pair[1].decode('utf-8'), False)
        thread_safe_queue.put(score)
...
df = pd.DataFrame()
df['nltk_similarity_score'] = pd.Series(all_scores)
df.to_csv(OUTPUT_NAME)
assert len(all_scores) == len(sentences1)
