...
from models.abshishek.lightgbm_booster import make_submission
...
def prepare_train():
    ...
    logreg_oof = np.load('logreg_train.csv.npy')
    df_train['logreg_oof'] = pd.Series(logreg_oof.ravel())

    magic1_cosine_extra = pd.read_csv(INPUT_FOLDER + '/features/magic_cosine_extra_train.csv', index_col=0)
    df_train = df_train.join(magic1_cosine_extra)

    siamese_stack = pd.read_csv(INPUT_FOLDER + '/features/siamese_oof_train.csv', index_col=0)
    df_train = df_train.join(siamese_stack)

    magicv2 = pd.read_csv('magicv2_train.csv', index_col=0)
    df_train = df_train.join(magicv2)
...
if __name__ == '__main__':
    ...
    base_classifiers = [LightGBMWrapper(params_lgb)]
    ...
    for clf_id, clf in enumerate(base_classifiers):
        ...
        for fold_id, (train_indexes, predict_indexes) in enumerate(splits):
            ...
            print 'Predict indexes', predict_indexes
    ...
    submission_pred = df_blend_test.mean(1)

    mydate = datetime.datetime.now()
    TODAY = str(mydate.today().date())[-2:] + '_' + str(mydate.today().month)
    make_submission('lgb_all_data_{0}_folds_{1}.csv'.format(N_FOLDS, TODAY), submission_pred)
